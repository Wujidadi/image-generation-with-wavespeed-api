---
updated: 2026-09-14T15:07:48+08:00
---
...
| ---------- | ---------------------------------------------------------------- | -------- | -------- | -------- |
| 最低限度   | 每個新模型 1 張，全用預設參數                                    | 40       | $1.740   | $1.716   |
| 最省設定   | 每個新模型 1 張，計費參數壓到最低檔                              | 40       | $1.612   | $1.593   |
| 含檔位驗證 | 預設參數各 1 張，另對 15 個有計費檔位的模型各加測 1 張非預設檔位 | 55       | $2.392   | $2.349   |
| 全高檔位   | 每個新模型 1 張，計費參數拉到最高檔                              | 40       | $3.239   | $3.190   |

單一模型最貴的一次呼叫是兩個 GPT Image 2.5 模型的 `quality=max`，各 $0.360；測試時務必維持預設的 `medium` 或降為 `low`。

### 實測結果

上表「最省設定」那一輪已實際執行：40 個新模型各產 1 張圖，多數用預設參數，五個模型改用非預設的最省檔位
...
後續若要比較各模型的輸出品質，每個模型多跑 1 輪預設參數約再花 $1.72。
