#!/usr/bin/env python3
"""以 WaveSpeed API 呼叫 wavespeed-ai/flux-dev 進行文生圖。

用法範例（於專案根目錄執行）：
    python3 scripts/flux-dev.py
    python3 scripts/flux-dev.py -p prompts/xxx.txt -o output/xxx --size 1280x720 --num-images 2
    python3 scripts/flux-dev.py --prompt "A cinematic shot of a city at sunset"
    python3 scripts/flux-dev.py --task-id <任務 ID> -o output/xxx
...
def add_model_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        "--guidance-scale", type=ws.number_type(1, 20, integer=False), default=3.5,
        help="提示詞遵循強度",
    )
