#!/usr/bin/env python3
"""以 WaveSpeed API 呼叫 openai/gpt-image-2.5-flare/text-to-image 進行文生圖。

用法範例（於專案根目錄執行）：
    python3 scripts/gpt-image-2.5-flare-text-to-image.py
    python3 scripts/gpt-image-2.5-flare-text-to-image.py -p prompts/xxx.txt -o output/xxx --quality low --resolution 1k
    python3 scripts/gpt-image-2.5-flare-text-to-image.py --prompt "A cinematic shot of a city at sunset"
    python3 scripts/gpt-image-2.5-flare-text-to-image.py --task-id <任務 ID> -o output/xxx
"""
...
MODEL_ID = "openai/gpt-image-2.5-flare/text-to-image"
ASPECT_RATIOS = [
