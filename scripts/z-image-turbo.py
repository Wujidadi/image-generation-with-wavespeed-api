#!/usr/bin/env python3
"""以 WaveSpeed API 呼叫 wavespeed-ai/z-image/turbo 進行文生圖。

用法範例（於專案根目錄執行）：
    python3 scripts/z-image-turbo.py
    python3 scripts/z-image-turbo.py -p prompts/xxx.txt -o output/xxx --size 1280x720 --output-format png
    python3 scripts/z-image-turbo.py --prompt "A cinematic shot of a city at sunset"
    python3 scripts/z-image-turbo.py --task-id <任務 ID> -o output/xxx
